...
export default class BillItems extends Component<{ bill: Bill }, {}> {
  ...
  private renderItemsFirstPage() {
    let itemCountToRender =
      this.props.bill.order.billItems!.length > this.maxItemsOnFirstPage
        ? this.maxItemsOnFirstPage
        : this.props.bill.order.billItems!.length;
  ...
  private renderTakeOver(start: number, end: number, breakPage?: boolean) {
    let takeOver = this.props.bill.order
      .billItems!.slice(start, end)
      .map((billitem: BillItem) => billitem.amount * billitem.price)
    ...
    return (
      <View style={[styles.row]} break={breakPage}>
        <Text style={[styles.column2, { marginLeft: 400, marginTop: 12 }]}>Übertrag</Text>
        <Text style={[styles.column2, styles.sum, { marginTop: 12 }]}>{takeOver}</Text>
      </View>
    );
  }
