...
export default class StatisticService {
  public static getSales(orders: Order[], startPeriod: Moment, endPeriod: Moment): Statistik[] {
    return orders
      ...
      .map((value) => {
        let s = new Statistik();
        s.date = moment(value.paymentRecievedDate, "DD.MM.YYYY");
        s.paid = value.billItems!.map((s) => s.amount * s.price).reduce((a, b) => a + b, 0) * (1 + value.taxRate);
        return s;
      });
  }
  ...
  public static getPendingBill(orders: Order[], startPeriod: Moment, endPeriod: Moment): Statistik[] {
    return orders
      ...
      .map((value) => {
        let s = new Statistik();
        s.date = moment(value.billDate, "DD.MM.YYYY");
        s.billed = value.billItems!.map((s) => s.amount * s.price).reduce((a, b) => a + b, 0) * (1 + value.taxRate);
        return s;
      });
  }
